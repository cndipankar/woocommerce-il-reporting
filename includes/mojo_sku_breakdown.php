...
function get_mojo_sku_breakdown($data1, $data2) {
	...
	if ( $orders ) {
		...
		foreach($orders as $o){
			...
			if (empty($orders_ar[$order_timestamp][$funnel]['orders'])){
				$orders_ar[$order_timestamp][$funnel]['orders'] = 1;
			} else {
				$orders_ar[$order_timestamp][$funnel]['orders']++;
			}
			...
			foreach($items as $item){
				...
				if ( empty($products_ar[$order_timestamp][$funnel][$sku]) ) {
					$products_ar[$order_timestamp][$funnel][$sku]['name'] = $sku_name;
					$products_ar[$order_timestamp][$funnel][$sku]['date'] = $order_date;
					$products_ar[$order_timestamp][$funnel][$sku]['orders'] = 0;
					$products_ar[$order_timestamp][$funnel][$sku]['units'] = 0;
					$products_ar[$order_timestamp][$funnel][$sku]['val_item'] = 0;
					$products_ar[$order_timestamp][$funnel][$sku]['val_shipping'] = 0;
				}

				$products_ar[$order_timestamp][$funnel][$sku]['orders']++;
				$products_ar[$order_timestamp][$funnel][$sku]['units'] = $products_ar[$order_timestamp][$funnel][$sku]['units'] + $qty;

				$products_ar[$order_timestamp][$funnel][$sku]['val_item'] = $products_ar[$order_timestamp][$funnel][$sku]['val_item'] + $orderItemValues['items'];
				$products_ar[$order_timestamp][$funnel][$sku]['val_shipping'] = $products_ar[$order_timestamp][$funnel][$sku]['val_shipping'] +  $orderItemShippingValues;
				$products_ar[$order_timestamp][$funnel][$sku]['val_tax'] = $products_ar[$order_timestamp][$funnel][$sku]['val_tax'] + $orderItemValues['tax'];	
	...
	foreach ( $products_ar as $kdata => $data1) {
		foreach ( $data1 as $kfunnel => $data2) {
			foreach ( $data2 as $ksku => $val) {

				$views = $orders_ar[$kdata][$kfunnel]['orders'];
...
add_action('template_redirect',function(){
	if ( class_exists( 'WooCommerce' ) ) {
		...
		if(is_product()){
			$pid = get_the_ID();
			$views = get_post_meta($pid,'visits_count',true);
			if(!$views){
				update_post_meta($pid,'visits_count',1);
			}else{
				$views++;
				update_post_meta($pid,'visits_count',$views);
			}
		}
	}
